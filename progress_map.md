...
- Latest completed step: Step 2.3 - narrow Minecraft/Create item model reader plus first 3D item previews in `SampleScene`.
- Latest verification: Unity batchmode was blocked on 2026-05-06 because another Unity instance had this project open, so no EditMode XML was produced. Compile-only fallback passed for the updated `Constructed.Unity` manifest/sync/catalog/model-loader/presenter files and the related `Constructed.Tests.EditMode` files using Unity's bundled mono/csc against the current script assemblies. A direct mono runner executed the new loader against the official Create reference and resolved all seven first-slice item models successfully: `andesite_alloy` and `belt_connector` as flat generated items, plus 3D parented models for `shaft`, `creative_motor`, `creative_crate`, `brass_funnel`, and `item_vault`.
- Next proposed step: Step 2.4 - start the block visual catalog by reading the simpler first-slice blockstate variant JSONs for shaft, creative motor, creative crate, brass funnel, and item vault, while explicitly deferring belt variant fan-out until after those blockstates are trusted.
...
Discuss and confirm Step 2.4 before implementation: start the block visual catalog by reading the simpler first-slice blockstate variant JSONs for shaft, creative motor, creative crate, brass funnel, and item vault, then use the existing model reader to place those block visuals in `SampleScene`. Defer belt blockstate fan-out, state-derived belt shape selection, kinetics, crate output, and funnel transfer behavior until the simpler blockstate path is trusted.
