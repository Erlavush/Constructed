...
- Latest completed step: Step 2.4 - first blockstate-driven block visual catalog in `SampleScene`.
- Latest verification: Unity EditMode batchmode completed successfully on 2026-05-06 using the standard `Unity.com -runTests -testPlatform EditMode` command. The step also passed targeted mono/csc compile fallback checks during development, and a direct mono runner resolved all five current block catalog variants through the new blockstate loader plus model loader against `References/Create-mc1.21.1-dev`.
- Next proposed step: Step 2.5 - replace the placeholder world cubes for the simpler first-slice blocks (`shaft`, `creative_motor`, `creative_crate`, `brass_funnel`, `item_vault`) with blockstate-driven visuals selected from authoritative runtime states, while keeping belts on placeholders until belt variant fan-out is handled.
...
Discuss and confirm Step 2.5 before implementation: replace the placeholder world cubes for `create:shaft`, `create:creative_motor`, `create:creative_crate`, `create:brass_funnel`, and `create:item_vault` with blockstate-driven visuals selected from authoritative runtime states, including the small property-alias/default bridge needed for `item_vault` and `brass_funnel`. Keep `create:belt` on placeholder visuals until belt blockstate fan-out and connection-shape selection are handled separately.
